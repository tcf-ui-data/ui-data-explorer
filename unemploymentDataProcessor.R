...
labor_force_info <- bind_rows(
  ...
  pivot_wider(names_from = metric, values_from = value) %>%
  # Print the values of labor_force_sa and labor_force_participation_rate_sa
  mutate(across(c(labor_force_sa, labor_force_participation_rate_sa), ~{
    message("labor_force_sa: ", .data$labor_force_sa)
    message("labor_force_participation_rate_sa: ", .data$labor_force_participation_rate_sa)
    .
  })) %>%
